...
async def client():
    """Async HTTP client with mocked token."""
...
class TestAuthentication:
    ...
    async def test_missing_auth_header(self, client):
        ...
        response = await client.post(
            "/config", json={"rho_max": 0.8, "energy_multiplier": 2.0}
        )
    ...
    async def test_invalid_token(self, client):
        ...
        response = await client.post(
            "/config", json={"rho_max": 0.8, "energy_multiplier": 2.0}, headers=headers
        )
        assert response.status_code == 401
        assert response.json()["detail"] == "unauthorized"
...
class TestConfigEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_valid_config_update(self, client, temp_config_file):
        """Test successful configuration update."""
        payload = {"rho_max": 0.85, "energy_multiplier": 3.0}

        with patch("agent.core.config_store._config_path", Path(temp_config_file)):
            response = await client.post("/config", json=payload, headers=HEADERS)
    ...
    @pytest.mark.asyncio
    async def test_invalid_rho_max_too_high(self, client):
    ...
    @pytest.mark.asyncio
    async def test_invalid_rho_max_too_low(self, client):
    ...
    @pytest.mark.asyncio
    async def test_invalid_energy_multiplier_too_high(self, client):
    ...
    @pytest.mark.asyncio
    async def test_invalid_energy_multiplier_too_low(self, client):
    ...
    async def test_audit_logging(
        self, mock_logger, mock_record, client, temp_config_file
    ):
...
class TestModelReloadEndpoint:
    ...
    @patch("agent.core.governor._SPECTRAL_MODEL")
    @pytest.mark.asyncio
    async def test_successful_model_reload(self, mock_model, client):
        """Test successful model reload."""
        # Mock the spectral radius estimation
        mock_model.estimate_spectral_radius.return_value = 0.75

        response = await client.post("/reload_model", headers=HEADERS)
    ...
    @pytest.mark.asyncio
    async def test_model_reload_metrics_and_logging(
        self, mock_counter, mock_logger, mock_record, mock_model, client
    ):
        """Test that model reload updates metrics and logs events."""
        # Mock the spectral radius estimation
        mock_model.estimate_spectral_radius.return_value = 0.65
    ...
    @patch("agent.core.governor._SPECTRAL_MODEL")
    @pytest.mark.asyncio
    async def test_model_reload_error_handling(self, mock_model, client):
        """Test error handling in model reload - test basic functionality."""
        # Mock the spectral radius estimation
        mock_model.estimate_spectral_radius.return_value = 0.55
...
class TestConfigIntegration:
    ...
    def test_config_validation_in_store(self):
        ...
        with pytest.raises(
            ValueError,
            match="Configuration validation failed.*Input should be less than 1",
        ):
            update_config({"rho_max": 1.5})

        with pytest.raises(
            ValueError,
            match="Configuration validation failed.*Input should be less than or equal to 4",
        ):
...
class TestExistingEndpoints:
    ...
    async def test_state_endpoint(self, client):
        ...
        response = await client.get("/state", headers=HEADERS)
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_pause_endpoint(self, client):
        """Test POST /pause endpoint."""
        response = await client.post("/pause", json={"flag": True}, headers=HEADERS)
        assert response.status_code == 200
        data = response.json()
    ...
    @pytest.mark.asyncio
    async def test_kill_endpoint(self, client):
        """Test POST /kill endpoint."""
        response = await client.post("/kill", headers=HEADERS)
        assert response.status_code == 200
        data = response.json()
        assert data["ok"] is True
