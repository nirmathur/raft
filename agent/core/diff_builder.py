...
def get_forbidden_patterns(charter: Dict[str, str] = None) -> List[re.Pattern]:
    ...
    for clause_id, clause_text in charter.items():
        ...
        if any(
            ...
            for keyword in ["forbidden", "dangerous", "no ", "block"]
        ):
...
class FunctionSignature:
    ...
    def to_smt_string(self) -> str:
        ...
        arg_types = " ".join(
            [f"String" for _ in self.args]
        )  # Simplified: all args as String
...
class GitDiffParser:
    ...
    def _extract_function_info(
        self, line: str, signatures: Dict[str, FunctionSignature], line_type: str
    ):
    ...
    def _detect_function_renames(
        self, added_lines: List[DiffLine], removed_lines: List[DiffLine]
    ) -> Dict[str, str]:
        ...
        for old_name, old_sig in removed_funcs.items():
            for new_name, new_sig in added_funcs.items():
                if (
                    ...
                    and new_name not in used_new_names
                    and old_sig.split("|")[0]
                ):  # Ensure non-empty args to avoid matching empty functions
        ...
        for old_name, old_sig in removed_funcs.items():
            for new_name, new_sig in added_funcs.items():
                if (
                    old_name == new_name  # Same name but different signature
                    and old_sig != new_sig
                    and new_name not in used_new_names
                    and old_name not in renames
                ):
                    renames[old_name] = new_name
                    used_new_names.add(new_name)
                    break
                    ...
                    if (
                        old_name in new_name
                        or new_name in old_name
                        or self._names_similar(old_name, new_name)
                    ):
                        renames[old_name] = new_name
                        used_new_names.add(new_name)
                        break
...
class SMTDiffBuilder:
        ...
        for line in diff_ast.added_lines:
            for pattern in self.forbidden_patterns:
                if pattern.search(line.content):
                    ...
                    logger.warning(
                        f"Forbidden pattern '{pattern.pattern}' found in: {line.content.strip()}"
                    )

        return violations

    def _build_forbidden_api_assertion(
        self, violations: List[Tuple[str, str, str, int]], ctx: z3.Context
    ) -> Optional[z3.BoolRef]:
    ...
    def _find_goal_preservation_violations(
        self, diff_ast: DiffAST
        ...
        for old_name, new_name in diff_ast.function_renames.items():
            ...
            if (
                old_sig_key in diff_ast.function_signatures
                and new_sig_key in diff_ast.function_signatures
            ):
                ...
                if len(old_sig.args) != len(new_sig.args) or str(old_sig.args) != str(
                    new_sig.args
                ):
                    violations.append(
                        (old_name, new_name, str(old_sig.args), str(new_sig.args))
                    )
                    logger.warning(
                        f"Function rename {old_name} -> {new_name} changes signature: {old_sig.args} != {new_sig.args}"
                    )
                else:
                    logger.info(
                        f"Function rename detected: {old_name} -> {new_name} with matching signatures"
                    )

        return violations

    def _build_goal_preservation_assertion(
        self,
        diff_ast: DiffAST,
...
def get_cached_proof(diff_hash: str) -> Optional[str]:
    ...
    try:
        ...
        logger.debug(
            f"Cache lookup for diff hash {diff_hash} failed: {e} - falling back to computation"
        )
...
def cache_proof_result(diff_hash: str, result: str) -> None:
    ...
    try:
        ...
        logger.debug(
            f"Cache store for diff hash {diff_hash} failed: {e} - result not cached"
        )
