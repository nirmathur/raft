...
class SimpleNet(nn.Module):
    ...
    Tip: use ``SimpleNet.create_stable_model(…, target_rho=0.8)`` if you
    need a network that always passes the RAFT spectral-radius guard.
    """

    def __init__(
        self, in_dim: int, out_dim: int, hidden_dim: int = 64, activation: str = "tanh"
    ):
        super().__init__()
        ...
        for module in self.modules():
            if isinstance(module, nn.Linear):
                ...
                nn.init.xavier_uniform_(
                    module.weight, gain=nn.init.calculate_gain(self._gain)
                )
        ...
        if batch_mode or x.dim() > 1:
            ...
            if x.dim() == 1:
                ...
                for i in range(batch_size):
                    x_i = x[i]
                    rho_i = self._estimate_single_spectral_radius(
                        x_i, n_iter, tolerance
                    )
    ...
    def _estimate_single_spectral_radius(
        self, x: torch.Tensor, n_iter: int, tolerance: float
    ) -> float:
    ...
    def create_stable_model(
        ...
        """Create a model with controlled spectral radius for production use.

        This method creates a model that is guaranteed to have a spectral radius
        below the target threshold, making it suitable for production systems.
        ...
        out_dim : int
            Output dimension
        target_rho : float
            Target spectral radius (should be < 0.9 for stability)
        method : str
            Initialization method ('xavier' or 'uniform')

        Returns
        -------
        SimpleNet
            Model with controlled spectral radius
        """
        model = cls(in_dim=in_dim, out_dim=out_dim, hidden_dim=32, activation="tanh")

        # Initialize with smaller weights to ensure stability
        for module in model.modules():
            if isinstance(module, nn.Linear):
                if method == "xavier":
                    ...
                    # Uniform initialization with small bounds
