...
class DiffGenerator:
    ...
    def generate_function_diff(
        self, old_name: str, new_name: str, args: List[str]
    ) -> str:
...
"""
    ...
    def generate_safe_diff(self, line_count: int, file_count: int = 1) -> str:
        ...
        for file_idx in range(file_count):
            ...
            lines_per_file = max(
                1, lines_per_file
            )  # Ensure every file has at least one line
    ...
    def generate_unsafe_diff(
        self, line_count: int, file_count: int = 1, forbidden_ratio: float = 0.1
    ) -> str:
        ...
        for file_idx in range(file_count):
            ...
            operations = [
                random.choice(self.SAFE_OPERATIONS)
                for _ in range(safe_count // file_count)
            ] + [
                random.choice(self.FORBIDDEN_OPERATIONS)
                for _ in range(forbidden_count // file_count)
            ]
...
class SMTBenchmarker:
    ...
    def benchmark_diff(self, diff_text: str, description: str) -> BenchmarkResult:
        ...
        self.log(
            f"  Time: {build_time:.4f}s, Result: {smt_result}, Risk: {result.risk_score:.3f}"
        )
        return result

    def benchmark_size_scaling(
        self, sizes: List[int], trials: int = 3
        ...
        for size in sizes:
            ...
            for trial in range(trials):
                ...
                result = self.benchmark_diff(
                    diff_text, f"Safe diff, {size} lines (trial {trial + 1})"
                )
    ...
    def benchmark_forbidden_patterns(
        self, line_count: int = 50
    ) -> List[BenchmarkResult]:
        ...
        for ratio in forbidden_ratios:
            if ratio == 0.0:
                ...
                diff_text = self.generator.generate_unsafe_diff(
                    line_count, forbidden_ratio=ratio
                )
                description = (
                    f"Unsafe diff, {line_count} lines, {ratio * 100:.0f}% forbidden"
                )
    ...
    def benchmark_function_renames(
        self, rename_counts: List[int]
    ) -> List[BenchmarkResult]:
        ...
        for count in rename_counts:
            ...
            result = self.benchmark_diff(
                diff_text, f"Function renames, {count} functions"
            )
            results.append(result)

        return results

    def benchmark_file_scaling(
        self, line_count: int = 100, file_counts: List[int] = None
    ) -> List[BenchmarkResult]:
        ...
        for file_count in file_counts:
            ...
            result = self.benchmark_diff(
                diff_text, f"Safe diff, {line_count} lines, {file_count} files"
            )
            results.append(result)
    ...
    def generate_report(
        ...
        all_times = [
            r.smt_build_time for results_list in results.values() for r in results_list
        ]
        if all_times:
        ...
        if "size_scaling" in results:
            ...
            if len(size_results) >= 2:
                ...
                if time_ratio < size_ratio:
                    insights.append(
                        f"SMT solver scales well: {size_ratio:.1f}x size increase → {time_ratio:.1f}x time increase"
                    )
                else:
                    insights.append(
                        f"SMT solver scaling concern: {size_ratio:.1f}x size increase → {time_ratio:.1f}x time increase"
                    )
        ...
        if "forbidden_patterns" in results:
            ...
            safe_times = [
                r.smt_build_time for r in forbidden_results if not r.has_forbidden
            ]
            unsafe_times = [
                r.smt_build_time for r in forbidden_results if r.has_forbidden
            ]
            ...
            if safe_times and unsafe_times:
                ...
                if ratio > 1.5:
                    insights.append(
                        f"Forbidden pattern detection adds overhead: {ratio:.1f}x slower than safe diffs"
                    )
                else:
                    insights.append(
                        f"Forbidden pattern detection is efficient: only {ratio:.1f}x slower than safe diffs"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Benchmark SMT diff analysis performance"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--quick",
        action="store_true",
        help="Run quick benchmark with smaller test cases",
    )
    ...
    print(
        f"Average time per benchmark: {report['summary']['avg_time_per_benchmark']:.4f}s"
    )
